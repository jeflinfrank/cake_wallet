...
import 'package:monero/src/beldex.dart' as api;
...
Future<void> enableLedgerExchange(LedgerConnection connection) async {
  ...
  void callback(Pointer<UnsignedChar> request, int requestLength) async {
    ...
    latestLedgerCommand = _ledgerBeldexCommands[ledgerRequest[1]];

    api.BeldexWallet.setDeviceReceivedData(
  ...
  api.BeldexWallet.setLedgerCallback(callable!.nativeFunction);
}
...
void _logLedgerCommand(Uint8List command, [bool isResponse = true]) {
  ...
  if (isResponse) {
    ...
    printV(
        "> ${_ledgerBeldexCommands[command[1]]} ${toHexString(command.sublist(2))}");
  }
}
