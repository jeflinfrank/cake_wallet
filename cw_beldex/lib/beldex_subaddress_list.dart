...
part 'beldex_subaddress_list.g.dart';

class BeldexSubaddressList = BeldexSubaddressListBase
    with _$BeldexSubaddressList;

abstract class BeldexSubaddressListBase with Store {
  BeldexSubaddressListBase()
    : _isRefreshing = false,
      _isUpdating = false,
  ...
  Future<void> updateWithAutoGenerate({
    ...
    try {
      ...
      refresh(accountIndex: accountIndex);
      subaddresses.clear();
      final newSubAddresses = await _getAllUnusedAddresses(
        accountIndex: accountIndex,
        label: defaultLabel,
      );
  ...
  Future<List<Subaddress>> _getAllUnusedAddresses({
    ...
    if (allAddresses.isEmpty ||
        _usedAddresses.contains(allAddresses.first.address)) {
      final isAddressUnused = await _newSubaddress(
        accountIndex: accountIndex,
        label: label,
      );
      if (!isAddressUnused) {
        return await _getAllUnusedAddresses(
          accountIndex: accountIndex,
          label: label,
        );
      }
