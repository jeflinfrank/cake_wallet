...
import 'package:cw_core/beldex_balance.dart';
import 'package:cw_core/beldex_transaction_priority.dart';
import 'package:cw_core/beldex_wallet_keys.dart';
...
import 'package:cw_beldex/api/coins_info.dart';
import 'package:cw_beldex/api/beldex_output.dart';
import 'package:cw_beldex/api/structs/pending_transaction.dart';
import 'package:cw_beldex/api/transaction_history.dart' as transaction_history;
import 'package:cw_beldex/api/wallet.dart' as beldex_wallet;
import 'package:cw_beldex/api/wallet_manager.dart';
import 'package:cw_beldex/exceptions/beldex_transaction_creation_exception.dart';
...
import 'package:cw_beldex/beldex_wallet_addresses.dart';
...
class BeldexWallet = BeldexWalletBase with _$BeldexWallet;

abstract class BeldexWalletBase
    extends
        WalletBase<
          BeldexBalance,
          BeldexTransactionHistory,
          BeldexTransactionInfo
        >
    with Store {
  BeldexWalletBase({
  ...
  }) : balance = ObservableMap<CryptoCurrency, BeldexBalance>.of({
         CryptoCurrency.bdx: BeldexBalance(
           fullBalance: beldex_wallet.getFullBalance(accountIndex: 0),
           unlockedBalance: beldex_wallet.getUnlockedBalance(accountIndex: 0),
         ),
       }),
    ...
    transactionHistory = BeldexTransactionHistory();
    walletAddresses = BeldexWalletAddresses(walletInfo, transactionHistory);
    ...
    _onAccountChangeReaction = reaction((_) => walletAddresses.account, (
      ...
      balance = ObservableMap<CryptoCurrency, BeldexBalance>.of(
        <CryptoCurrency, BeldexBalance>{
          currency: BeldexBalance(
            fullBalance: beldex_wallet.getFullBalance(accountIndex: account.id),
            unlockedBalance: beldex_wallet.getUnlockedBalance(
    ...
    _onTxHistoryChangeReaction = reaction((_) => transactionHistory, (__) {
      _updateSubAddress(
        isEnabledAutoGenerateSubaddress,
  ...
  @override
  late BeldexWalletAddresses walletAddresses;
  ...
  @override
  @observable
  ObservableMap<CryptoCurrency, BeldexBalance> balance;

  @override
  bool get hasRescan => true;

  @override
  String get seed => beldex_wallet.getSeed();
  String seedLegacy(String? language) => beldex_wallet.getSeedLegacy(language);

  @override
  String get password => _password;

  @override
  String get passphrase => beldex_wallet.getPassphrase();

  @override
  BeldexWalletKeys get keys => BeldexWalletKeys(
    primaryAddress: beldex_wallet.getAddress(accountIndex: 0, addressIndex: 0),
    ...
    passphrase: beldex_wallet.getPassphrase(),
  );
  ...
  List<BeldexUnspent> unspentCoins;
  String _password;
  ...
  Future<void> init() async {
    ...
    balance = ObservableMap<CryptoCurrency, BeldexBalance>.of(
      <CryptoCurrency, BeldexBalance>{
        currency: BeldexBalance(
          fullBalance: beldex_wallet.getFullBalance(
    ...
    if (walletInfo.isRecovery) {
      beldex_wallet.setRecoveringFromSeed(isRecovery: walletInfo.isRecovery);

      if (beldex_wallet.getCurrentHeight() <= 1) {
        beldex_wallet.setRefreshFromBlockHeight(
    ...
    walletAddresses.subaddressList.update(
  ...
  Future<bool> checkNodeHealth() async {
    try {
      ...
      final isConnected = await beldex_wallet.isConnected();
      ...
      final nodeHeight = await beldex_wallet.getNodeHeight();
      return nodeHeight > 0;
    } catch (e) {
      return false;
    }
  ...
  Future<void> close({bool shouldCleanup = false}) async {
    if (isHardwareWallet) {
      ...
      final currentWalletDirPath = await pathForWalletDir(
        name: name,
        type: type,
      );
      if (openedWalletsByPath["$currentWalletDirPath/$name"] != null) {
        ...
        final waddr =
            openedWalletsByPath["$currentWalletDirPath/$name"]!.ffiAddress();
  ...
  Future<void> connectToNode({required Node node}) async {
    ...
    try {
      ...
      await beldex_wallet.setupNodeSync(
      ...
      await beldex_wallet.setTrustedDaemon(node.trusted);
  ...
  Future<void> startSync() async {
    try {
      ...
      try {
        await resetCache(
          name,
        ); // Resetting the cache removes the TX Keys and Polyseed
        _assertInitialHeight();
      } catch (e) {
  ...
  Future<bool> submitTransactionUR(String ur) async {
    ...
    if (status != 0) {
      final err = currentWallet!.errorString();
      throw BeldexTransactionCreationException(
        "unable to broadcast signed transaction: $err",
      );
    }
    return retStatus;
  }
  ...
  Map<String, String> exportOutputsUR() {
    ...
    return {"Outputs (partial)": str, "Outputs (all)": strAll};
  }
  ...
  Future<PendingTransaction> createTransaction(Object credentials) async {
    ...
    final unlockedBalance = beldex_wallet.getUnlockedBalance(
      accountIndex: walletAddresses.account!.id,
    );
    ...
    if (!(syncStatus is SyncedSyncStatus)) {
      throw BeldexTransactionCreationException('The wallet is not synced.');
    }
    ...
    if (hasMultiDestination) {
      if (outputs.any(
        ...
        throw BeldexTransactionCreationException(
          'You do not have enough BDX to send this amount.',
        );
      }

      final int totalAmount = outputs.fold(
        0,
        (acc, value) => acc + (value.formattedCryptoAmount ?? 0),
      );
      ...
      if (inputs.isEmpty)
        BeldexTransactionCreationException('No inputs selected');
      ...
      pendingTransactionDescription = await transaction_history
          .createTransactionMultDest(
            outputs: beldexOutputs,
      ...
      //   final formattedBalance = beldexAmountToString(amount: unlockedBalance);
      //
      //   throw BeldexTransactionCreationException(
    ...
    // final status = beldex.PendingTransaction_status(pendingTransactionDescription);

    return PendingBeldexTransaction(pendingTransactionDescription, this);
  }
  ...
  int calculateEstimatedFee(TransactionPriority priority, int? amount) {
    ...
    if (priority is BeldexTransactionPriority) {
      switch (priority) {
  ...
  Future<void> renameWalletFiles(String newWalletName) async {
    ...
    try {
      ...
      final currentWalletDir = Directory(
      ...
      if (currentAddressListFile.existsSync()) {
        await currentAddressListFile.copy(
          "${newWalletDirPath}/$newWalletName.address.txt",
        );
      }
      if (backgroundSyncFile.existsSync()) {
        await backgroundSyncFile.copy(
          "${newWalletDirPath}/$newWalletName.background",
        );
      }
      ...
      final newWalletPath = await pathForWallet(
        name: newWalletName,
        type: type,
      );
  ...
  Future<void> changePassword(String password) async =>
      beldex_wallet.setPasswordSync(password);

  Future<int> getNodeHeight() async => beldex_wallet.getNodeHeight();
  ...
  Future<void> updateUnspent() async {
    try {
      ...
      for (var i = 0; i < coinCount; i++) {
        ...
        if (coinSpent == false &&
            coin.subaddrAccount() == walletAddresses.account!.id) {
          final unspent = await BeldexUnspent.fromUnspent(
  ...
  Future<void> _addCoinInfo(BeldexUnspent coin) async {
    ...
    try {
      ...
      if (currentWalletUnspentCoins.isNotEmpty) {
        currentWalletUnspentCoins.forEach((element) {
          final existUnspentCoins = unspentCoins.where(
            (coin) => element.keyImage!.contains(coin.keyImage!),
          );
  ...
  String getTransactionAddress(int accountIndex, int addressIndex) =>
      beldex_wallet.getAddress(
        accountIndex: accountIndex,
        addressIndex: addressIndex,
      );

  @override
  Future<Map<String, BeldexTransactionInfo>> fetchTransactions() async {
    await transaction_history.refreshTransactions();
    final resp = (await _getAllTransactionsOfAccount(
      walletAddresses.account?.id,
    )).fold<Map<String, BeldexTransactionInfo>>(
      <String, BeldexTransactionInfo>{},
      (Map<String, BeldexTransactionInfo> acc, BeldexTransactionInfo tx) {
        acc[tx.id] = tx;
        return acc;
      },
    );
  ...
  Future<void> updateTransactions() async {
    try {
      ...
      currentIds
          .difference(newIds)
          .forEach((id) => transactionHistory.transactions.remove(id));

      // Add or update transactions
      transactions.forEach(
        (key, tx) => transactionHistory.transactions[key] = tx,
      );
  ...
  String getSubaddressLabel(int accountIndex, int addressIndex) =>
      beldex_wallet.getSubaddressLabel(accountIndex, addressIndex);

  Future<List<BeldexTransactionInfo>> _getAllTransactionsOfAccount(
    int? accountIndex,
  ) async =>
      (await transaction_history.getAllTransactions())
          .map(
            (row) => BeldexTransactionInfo(
                row.hash,
                row.blockheight,
                row.isSpend
  ...
  void _assertInitialHeight() {
    ...
    final height = beldex_wallet.getCurrentHeight();
  ...
  int _getHeightDistance(DateTime date) {
    ...
    return days * 720; // there are720 blocks per day on bdx
  }
  ...
  void _askForUpdateBalance() {
    ...
    final fullBalance = beldex_wallet.getFullBalance(
      accountIndex: walletAddresses.account!.id,
    );
    ...
    if (balance[currency]!.fullBalance != fullBalance ||
      ...
      balance[currency] = BeldexBalance(
  ...
  int _getUnlockedBalance() => beldex_wallet.getUnlockedBalance(
    accountIndex: walletAddresses.account!.id,
  );
  ...
  int _getFrozenBalance() {
    ...
    for (var coin in unspentCoinsInfo.values.where(
      (element) =>
          element.walletId == id &&
          element.accountIndex == walletAddresses.account!.id,
    )) {
  ...
  Future<String> signMessage(String message, {String? address}) async {
    ...
    return beldex_wallet.signMessage(message, address: useAddress);
  }
  ...
  Future<bool> verifyMessage(
    ...
    return beldex_wallet.verifyMessage(message, address, signature);
  }
  ...
  String formatCryptoAmount(String amount) {
    return beldexAmountToString(amount: int.parse(amount));
  }
}
