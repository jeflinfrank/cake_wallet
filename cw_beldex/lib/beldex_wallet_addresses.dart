...
part 'beldex_wallet_addresses.g.dart';

class BeldexWalletAddresses = BeldexWalletAddressesBase
    with _$BeldexWalletAddresses;

abstract class BeldexWalletAddressesBase extends WalletAddresses with Store {
  BeldexWalletAddressesBase(
    WalletInfo walletInfo,
    BeldexTransactionHistory beldexTransactionHistory,
  ) : accountList = BeldexAccountList(),
      _beldexTransactionHistory = beldexTransactionHistory,
      subaddressList = BeldexSubaddressList(),
      address = '',
      super(walletInfo);

  final BeldexTransactionHistory _beldexTransactionHistory;
  @override
  @observable
  String address;
  ...
  BeldexSubaddressList subaddressList;

  BeldexAccountList accountList;
  ...
  Future<void> updateAddressesInBox() async {
    try {
      final _subaddressList = BeldexSubaddressList();
  ...
  void updateSubaddressList({required int accountIndex}) {
    ...
    address =
        subaddressList.subaddresses.isNotEmpty
            ? subaddressList.subaddresses.first.address
            : getAddress();
  }

  Future<void> updateUsedSubaddress() async {
    final transactions = _beldexTransactionHistory.transactions.values.toList();
  ...
  Future<void> updateUnusedSubaddress({
    ...
    subaddress =
        (subaddressList.subaddresses.isEmpty)
            ? Subaddress(
              id: 0,
              address: address,
              label: defaultLabel,
    ...
    if (num.tryParse(subaddress!.balance ?? '0') != 0) {
      getAddress(
        accountIndex: accountIndex,
        addressIndex: (subaddress?.id ?? 0) + 1,
      );
    }
    address = subaddress!.address;
  }
