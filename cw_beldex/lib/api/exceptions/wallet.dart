...
import 'package:cw_beldex/api/exceptions/setup_wallet_exception.dart';
import 'package:cw_beldex/api/get_all_unspent.dart';
...
String getSeed() {
  // beldex.Wallet_setCacheAttribute(wptr!, key: "cakewallet.seed", value: seed);
  ...
  if (cakepolyseed != "") {
    if (cakepassphrase != "") {
      try {
        ...
        if (ps.isEncrypted || cakepassphrase == "")
...
String getAddress({int accountIndex = 0, int addressIndex = 0}) {
  ...
  if (currentWallet == null)
    return "<wallet not ready ($accountIndex:$addressIndex)>";

  // printV("getaddress: ${accountIndex}/${addressIndex}: ${beldex.Wallet_numSubaddresses(wptr!, accountIndex: accountIndex)}: ${beldex.Wallet_address(wptr!, accountIndex: accountIndex, addressIndex: addressIndex)}");
  // this could be a while loop, but I'm in favor of making it if to not cause freezes
  if (currentWallet!.numSubaddresses(accountIndex: accountIndex) - 1 <
      addressIndex) {
  ...
  addressCache[currentWallet!
      .ffiAddress()]![accountIndex]![addressIndex] ??= currentWallet!.address(
    accountIndex: accountIndex,
    addressIndex: addressIndex,
  );
  return addressCache[currentWallet!
      .ffiAddress()]![accountIndex]![addressIndex]!;
}
    ...
    (() async {
      try {
        ...
        cachedNodeHeight = await Isolate.run(() async {
          return beldex.Wallet_daemonBlockChainHeight(
            Pointer.fromAddress(wptrAddress),
          );
        });
      } catch (_) {
      } finally {
...
Future<bool> isConnected() async {
  ...
  return await Isolate.run(
        () => beldex.Wallet_connected(Pointer.fromAddress(wptrAddress)),
      ) ==
      1;
}
...
void storeSync({bool force = false}) async {
  ...
  final synchronized = await Isolate.run(() {
    return beldex.Wallet_synchronized(Pointer.fromAddress(addr));
  });
...
class SyncListener {
  ...
  void _start() {
    ...
    _lastKnownBlockHeight = 0;
    _initialSyncHeight = 0;
    _updateSyncInfoTimer ??= Timer.periodic(Duration(milliseconds: 1200), (
      _,
    ) async {
...
bool verifyMessage(String message, String address, String signature) {
  return currentWallet!.verifySignedMessage(
    message: message,
    address: address,
    signature: signature,
  );
...
Map<String, List<int>> debugCallLength() => beldex.debugCallLength;

Map<String, dynamic> getWalletCacheDebug() {
  try {
    final jsonString = beldex.BELDEX_Wallet_serializeCacheToJson(
