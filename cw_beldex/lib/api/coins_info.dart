...
Future<void> refreshCoins(int accountIndex) async {
  ...
  await Isolate.run(() => beldex.Coins_refresh(Pointer.fromAddress(coinsPtr)));
  coinsMutex.release();
}
...
Future<void> freezeCoin(int index) async {
  ...
  final coinsPtr = coins!.ffiAddress();
  await Isolate.run(
    () => beldex.Coins_setFrozen(Pointer.fromAddress(coinsPtr), index: index),
  );
  coinsMutex.release();
}
...
Future<void> thawCoin(int index) async {
  ...
  final coinsPtr = coins!.ffiAddress();
  await Isolate.run(
    () => beldex.Coins_thaw(Pointer.fromAddress(coinsPtr), index: index),
  );
  coinsMutex.release();
}
