...
import 'package:monero/beldex.dart';

bool isUpdating = false;

class SubaddressInfoMetadata {
  SubaddressInfoMetadata({required this.accountIndex});
  int accountIndex;
}
...
class Subaddress {
  ...
  String get label {
    final localLabel =
        currentWallet?.getSubaddressLabel(
          accountIndex: accountIndex,
          addressIndex: addressIndex,
        ) ??
        "";
...
class TinyTransactionDetails {
  TinyTransactionDetails({required this.address, required this.amount});
...
List<Subaddress> getAllSubaddresses() {
        ...
        TinyTransactionDetails(
          address: List.generate(
            ...
            (index) => getAddress(
              ...
              addressIndex: int.tryParse(subaddrs[index]) ?? 0,
            ),
          ),
          amount: tx.amount(),
        ),
  ...
  final size = currentWallet!.numSubaddresses(
    accountIndex: subaddress!.accountIndex,
  );
  final list =
      List.generate(size, (index) {
        final ttDetailsLocal =
            ttDetails.where((element) {
