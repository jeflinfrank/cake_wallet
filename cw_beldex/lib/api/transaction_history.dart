...
import 'package:cw_beldex/api/exceptions/creation_transaction_exception.dart';
...
import 'package:monero/src/generated_bindings_beldex.g.dart' as beldex_gen;
...
Future<void> refreshTransactions() async {
  ...
  await Isolate.run(() {
    beldex.TransactionHistory_refresh(Pointer.fromAddress(ptr));
  });
...
Future<List<Transaction>> getAllTransactions() async {
  ...
  for (int index = 0; index < size; index++) {
    ...
    if (txhistory!.ffiAddress() * currentWallet!.ffiAddress() != startAddress) {
      printV(
        "Loop broken because txhistory!.address * wptr!.address != startAddress",
      );
      break;
    }
...
Future<PendingTransactionDescription> createTransactionSync({
  ...
  Isolate.run(() async {
    beldex.Wallet_synchronized(Pointer.fromAddress(waddr));
  });
  ...
  final preferredInputs_ =
      preferredInputs.join(beldex.defaultSeparatorStr).toNativeUtf8();
  ...
  final Wallet2PendingTransaction pendingTx = BeldexPendingTransaction(
    pendingTxPtr,
  );
  ...
  if (error != null) {
    ...
    if (message.contains("RPC error")) {
      message =
          "Invalid node response, please try again or switch node\n\ntrace: $message";
    }
...
Future<PendingTransactionDescription> createTransactionMultDest({
  required List<BeldexOutput> outputs,
  required String paymentId,
  required int priorityRaw,
  int accountIndex = 0,
  List<String> preferredInputs = const [],
  ...
  final amounts =
      outputs.map((e) => currentWallet!.amountFromString(e.amount)).toList();
  ...
  final txptr = Pointer<Void>.fromAddress(
    await Isolate.run(() {
      return beldex.Wallet_createTransactionMultDest(
        Pointer.fromAddress(waddr),
        dstAddr: dstAddrs,
        isSweepAll: false,
        amounts: amounts,
  ...
  final Wallet2PendingTransaction tx = BeldexPendingTransaction(txptr);
...
Future<String?> commitTransactionFromPointerAddress({
  required int address,
  required bool useUR,
}) => commitTransaction(
  tx: BeldexPendingTransaction(Pointer.fromAddress(address)),
  useUR: useUR,
);

Future<String?> commitTransaction({
  required Wallet2PendingTransaction tx,
  required bool useUR,
}) async {
  final txCommit =
      useUR
          ? tx.commitUR(120)
          : await Isolate.run(() {
            beldex.PendingTransaction_commit(
...
class Transaction {
  ...
  Transaction({required this.txInfo})
    : displayLabel = txInfo.label(),
      ...
      isSpend =
          txInfo.direction() == beldex.TransactionInfo_Direction.Out.index,
