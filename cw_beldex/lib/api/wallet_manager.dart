...
import 'package:cw_beldex/api/exceptions/wallet_creation_exception.dart';
import 'package:cw_beldex/api/exceptions/wallet_opening_exception.dart';
import 'package:cw_beldex/api/exceptions/wallet_restore_from_keys_exception.dart';
import 'package:cw_beldex/api/exceptions/wallet_restore_from_seed_exception.dart';
...
void checkIfMoneroCIsFine() {
  final checksum = BeldexWalletChecksum();
  ...
  final dartCsCpp = beldex.wallet2_api_c_cpp_sha256;
  final dartCsH = beldex.wallet2_api_c_h_sha256;
  final dartCsExp = beldex.wallet2_api_c_exp_sha256;

  if (cppCsCpp != dartCsCpp) {
    throw MoneroCException(
      "monero_c and beldex.dart cpp wrapper code mismatch.\nLogic errors can occur.\nRefusing to run in release mode.\ncpp: '$cppCsCpp'\ndart: '$dartCsCpp'",
    );
  }

  if (cppCsH != dartCsH) {
    throw MoneroCException(
      "monero_c and beldex.dart cpp wrapper header mismatch.\nLogic errors can occur.\nRefusing to run in release mode.\ncpp: '$cppCsH'\ndart: '$dartCsH'",
    );
  }

  if (cppCsExp != dartCsExp && (Platform.isIOS || Platform.isMacOS)) {
    throw MoneroCException(
      "monero_c and beldex.dart wrapper export list mismatch.\nLogic errors can occur.\nRefusing to run in release mode.\ncpp: '$cppCsExp'\ndart: '$dartCsExp'",
    );
  }
}
...
Wallet2WalletManager wmPtr =
    (() {
      try {
        ...
        beldex.printStarts = false;
        if (kDebugMode && debugBeldex) {
          BeldexWalletManagerFactory().setLogLevel(4);
        }
        _wmPtr ??= BeldexWalletManagerFactory().getWalletManager();
        if (kDebugMode && debugBeldex) {
          BeldexWalletManagerFactory().setLogLevel(4);
        }
        printV("ptr: $_wmPtr");
...
void restoreWalletFromSpendKeySync({
  ...
  // txhistory = null;
  // wptr = beldex.WalletManager_createWalletFromKeys(
  //   wmPtr,
  //   path: path,
  //   password: password,
  //   restoreHeight: restoreHeight,
...
Future<void> restoreWalletFromHardwareWallet({
  ...
  txhistory = null;
  final wmPtr = BeldexWalletManagerFactory().getWalletManager().ffiAddress();
  final newWptrAddr = await Isolate.run(() {
    return beldex.WalletManager_createWalletFromDevice(
      Pointer.fromAddress(wmPtr),
      path: path,
      password: password,
      restoreHeight: restoreHeight,
  ...
  final newW = BeldexWallet(Pointer.fromAddress(newWptrAddr));
...
Future<void> loadWallet({
  ...
  if (currentWallet == null || path != _lastOpenedWallet) {
    if (currentWallet != null) {
      ...
      Isolate.run(() {
        beldex.Wallet_store(Pointer.fromAddress(addr));
      });
    }
    txhistory = null;
    ...
    if (deviceType == 1) {
      if (gLedger == null) {
        throw Exception(
          "Tried to open a ledger wallet with no ledger connected",
        );
      }
    ...
    final newWptrAddr = await Isolate.run(() {
      return beldex.WalletManager_openWallet(
...
Future<void> openWallet({
  ...
  required String password,
  int nettype = 0,
}) async => loadWallet(path: path, password: password, nettype: nettype);
